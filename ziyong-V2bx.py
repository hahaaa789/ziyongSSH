...
def _nft_rule_block(tag, family, rules):
    b = []
    # nft -f 对已存在的 table 是追加而非替换 (ArchWiki: "any rules already
    # loaded are not automatically flushed"), 重复 load 会把同一条规则叠加多次.
    # 先声明空表再删除: 表不存在时声明不报错, 表存在时被删掉, 之后重建 -> 幂等.
    b.append("table %s hy2hop%s" % (family, tag))
    b.append("delete table %s hy2hop%s" % (family, tag))
    b.append("table %s hy2hop%s {" % (family, tag))
...
def migrate_old_nft_conf():
    """v0.7 迁移: 清掉 v0.6 及更早版本在 /etc/nftables.conf 里留下的 include 行.

    旧版把跳跃规则 include 进 Debian 默认的 /etc/nftables.conf 并 enable
    nftables.service. 那份文件开头是 flush ruleset, 会清空【整个】ruleset,
    连 iptables-nft 的 table ip nat (中转转发规则) 一起清掉.
    现在改用独立的 hy2hop.service, 这里负责把旧的残留拆掉.

    安全判据: 只有当去掉 include 行后文件里【没有任何实质规则】时,
    才 disable nftables.service. 否则说明用户自己在用它做防火墙,
    只删 include 行, 绝不动服务."""
    if not os.path.exists(NFT_CONF):
        return
    try:
        with open(NFT_CONF, "r", encoding="utf-8", errors="ignore") as f:
            old = f.readlines()
    except Exception as e:
        warn("读 %s 失败, 跳过迁移: %s" % (NFT_CONF, e))
        return
    kept, dropped = [], []
    for ln in old:
        s = ln.strip()
        if s.startswith("include") and "nftables.d" in s:
            dropped.append(s)
        else:
            kept.append(ln)
    if not dropped:
        return
    print("")
    warn("检测到旧版残留: %s 里的 include 行" % NFT_CONF)
    for s in dropped:
        info("  " + s)
    info("旧版会让 nftables.service 的 flush ruleset 清掉中转规则, 现在拆掉它")
    backup_file(NFT_CONF)
    try:
        with open(NFT_CONF, "w") as f:
            f.writelines(kept)
    except Exception as e:
        warn("写 %s 失败, 迁移中止: %s" % (NFT_CONF, e))
        return
    ok("已移除 include 行")
    real = 0
    for ln in kept:
        ...
        if not s or s.startswith("#") or s.startswith("!"):
            continue
        if s in ("flush ruleset",):
            continue
        real += 1
    if real == 0:
        run("systemctl disable --now nftables >/dev/null 2>&1",
            check=False, timeout=30)
        ok("nftables.service 已停用 (该文件已无实质规则)")
    else:
        warn("%s 里还有 %d 行自定义规则, 保留 nftables.service 不动" % (NFT_CONF, real))
        warn("注意: 该服务启动时的 flush ruleset 仍可能清掉中转规则, 请自行确认")
